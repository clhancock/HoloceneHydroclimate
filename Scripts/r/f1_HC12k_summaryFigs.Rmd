...
write.csv(proxyMetaData,file=file.path(githubDir,'Data',paste('proxyMetaData_',climVar,'.csv',sep='')))
##############################
...
pieData[which(pieData$source=='Oxford Lake Levels Database')
        ,'source'] <- 'Lake-Level Database (Street-Perrott et al., 1989)'
...
library(ggrepel)
z <- ggplot(pieData2,aes(x="", y=n, fill=source)) +
  ...
  scale_fill_manual(values=c("powder blue","dark blue","corn flower blue","forest green",
                             "grey","firebrick","yellowgreen","grey40","White"),name='Data Source') +
  theme_void() + 
...
ggsave(plot=z , width = figS[1], height = figS[2], dpi = 600,
       filename = file.path(githubDir,"Figures","PlotProxySource.png"))
...
for (i in 1:length(plotVals$specific$names)){
  proxyMapSites <- proxyMapSites +
    geom_star(data=(as.data.frame(proxyDataPrj) %>% filter(CategorySpec==plotSettings$names[i])),
              ...
              size=4,color='Black',alpha=0.7,starstroke=0.5)
