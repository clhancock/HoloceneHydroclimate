...
for (tso in lipdTSO){
  for (ka in c(0.5,6)){
    proxyMetaData[which(proxyMetaData$tsid == tso$paleoData_TSid),paste('ka',ka,sep='_')] <-
      round(mean(tso$paleoData_values[which(between(tso$age,1000*ka-500,1000*ka+500))],na.rm = TRUE),3)
  }
}

write.csv(proxyMetaData,file=file.path(dataDir,'Data',paste('proxyMetaData_',climVar,'.csv',sep='')))
```
