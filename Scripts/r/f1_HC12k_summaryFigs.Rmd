...
plotVals$general$shapes<- c(23,20,21,'p',22,'P',24)
...
ggsave(plot=proxyMap, width = 6, height = (6/1.97+0.5), dpi = 600,
       filename = file.path(githubDir,"Figures",paste('countByRegion_',climVar,'.png',sep='')))
```
...
```{r}
sdfProxy <- as.data.frame(proxyDataPrj)
proxyMap2 <- ggplot() +
  #Set Border around plot - probably not the best way to do this
  borders(aggregate(refregions, FUN=length), fill=NA, colour='black', size=2) +
  geom_map(data=refregions, map=fortify(refregions),
  ...
  #Add Country data (basemap)
  geom_map(data=countries, map=fortify(countries),
           aes(x=long, y=lat, group=group, map_id=id), fill = "grey90",color="white",size=0.2) +
  #Add refrence regions boundaries
  geom_map(data = refregions, map = dataTable, aes(x=long, y=lat, group=group, map_id=id),
           fill=NA, color='Black') +
...
plotVals$general$shapes<- c(5,1,10,9,0,3,2)
#"Glacier Ice""Lake Deposits","Lake Sediment d18O" "Leaf Wax dD","Other","Pollen","Speleothem"           
for (i in 1:length(plotVals$general$names)){
  proxyMap2 <- proxyMap2 +
    geom_point(data=(sdfProxy %>% filter(Category==plotVals$general$names[i])),
               aes(x=longitude.1 , y=latitude.1), size=0.5, stroke=1.5, alpha=0.8,
               shape=plotVals$general$shapes[i], color='Black')+
    geom_point(data=(sdfProxy %>% filter(Category==plotVals$general$names[i])),
               aes(x=longitude.1 , y=latitude.1), size=0.5, stroke=1, alpha=0.8,
               shape=plotVals$general$shapes[i], color=plotVals$general$colors[i])
}
