...
for (model in c('trace','hadcm','cmip')){
  for (var in c('p-e_ANN','pre_ANN','tas_ANN','p-e_JJA','pre_JJA','tas_JJA')){
    regionData[[model]][[var]] <- read.csv(file.path(dataDir,'Data','Model',
                                                     paste(var,'_',model,'.csv',sep='')))
  }
}
...
for (reg in regNames){
  ...
  regEnsNA <- regEnsNA - as.numeric(apply(regEnsNA[1:10,],2,mean,na.rm=TRUE))
  regEnsNA <- regEnsNA
  regEns    <- regEnsNA
  ...
  if (!is.na(modelVar)){
      ...
      hadcmTS$values <- 30*(hadcmTS$values - mean(hadcmTS$values[1:10],na.rm = TRUE))#/mean(hadcmTS$values[1:10],na.rm = TRUE) #/ ((hadcmTS$values[1])/1)
      compBands$vals <- compBands$vals + 
        geom_line(aes(x=hadcmTS$time,y=hadcmTS$values),color='#827717',size=0.4,alpha=0.8)
      #
      traceTS <- pltData[[reg]][['trace']]
      traceTS$values <- 30*(traceTS$values - mean(traceTS$values[1:10],na.rm = TRUE))#/mean(traceTS$values[1:10],na.rm = TRUE)
      compBands$vals <- compBands$vals + 
        geom_line(aes(x=traceTS$time,y=traceTS$values),color='firebrick4',size=0.4,alpha=0.8)
      compBands$vals <- compBands$vals + geom_boxplot(aes(x=6000,y=pltData[[reg]][['cmip']]*30),width=1000,fill=NA)
...
ggsave(plot=compBandPlt, width = 19, height = 9, dpi = 600,
           filename = paste(file.path(dataDir,'Figures','global'),modelVar,'_compBandPlt.png',sep=''))
```
