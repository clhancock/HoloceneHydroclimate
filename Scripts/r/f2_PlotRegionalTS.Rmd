...
for (model in c('trace','hadcm','cmip')){
  for (var in c('p-e_ANN','pre_ANN','tas_ANN')){
...
for (reg in regNames){
  ...
  if (!is.na(modelVar)){
      ...
      hadcmTS$values <- 1*(hadcmTS$values - hadcmTS$values[1]) #/ ((hadcmTS$values[1])/1)
      compBands$vals <- compBands$vals + 
        geom_line(aes(x=hadcmTS$time,y=hadcmTS$values),color='darkgreen',size=0.4,alpha=0.8)
      #
      traceTS <- pltData[[reg]][['trace']]
      traceTS$values <- 1*(traceTS$values - traceTS$values[1])# / ((traceTS$values[1])/1)
      compBands$vals <- compBands$vals + 
        geom_line(aes(x=traceTS$time,y=traceTS$values),color='firebrick4',size=0.4,alpha=0.8)
  }
  compBands$vals <- compBands$vals + geom_boxplot(aes(x=6000,y=pltData[[reg]][['cmip']]),width=1000)
  #compBands$vals
  compBandPlt <- ggdraw() 
...
ggsave(plot=compBandPlt, width = 19, height = 9, dpi = 600,
           filename = paste(file.path(dataDir,'Figures','global'),climVar,'_compBandPlt.png',sep=''))
```
