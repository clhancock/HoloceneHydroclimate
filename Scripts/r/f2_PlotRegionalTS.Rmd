...
ggsave(plot=StripePlot, width = 6, height = 6*0.5072, dpi = 600,
           filename = paste(file.path(githubDir,'Figures',''),'global_',climVar,'_StripePlot.png',sep=''))
...
for (reg in regNames){
  ...
  if (length(idx > 0)){
    idx <- sort(unique(idx[which(between(idx,1,length(binvec)))]))
    regionData[idx,] <- NA 
  }
  ...
  for (plt in names(compBands)){
    compBands[[plt]] <- compBands[[plt]] + 
      ...
      scale_y_continuous(name=paste('Standardized',climVar),
                      ...
                      breaks = seq(-plotlimit_set,plotlimit_set,2))+
...
ggsave(plot=compBandPlt, width = 6, height = 6*0.5072, dpi = 600,
           filename = paste(file.path(githubDir,'Figures',''),'global_',climVar,'_compBandPlt.png',sep=''))
```
