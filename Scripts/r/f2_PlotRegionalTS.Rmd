...
for (reg in regNames){
  ...
  compBands$na   <- plotTimeseriesEnsRibbons(X=binvec, Y=regEnsNA, alp=0.9,line.width=0.2,
                                             color.low='grey90',
                                             color.high='grey50',
                                             color.line='grey20') 
  compBands$vals <- plotTimeseriesEnsRibbons(X=binvec, Y=regEns, alp=0.9,line.width=0.2,
                                             color.low=climVarSettings[[climVar]]$Col[1],
                                             color.high=climVarSettings[[climVar]]$Col[2],
                                             color.line=climVarSettings[[climVar]]$Col[3]) 
  if (!is.na(modelVar)){
    for (model in c('trace','hadcm')){
      modelTS <- pltData[[reg]][[model]]
      modelTS$values <- modelTS$values - mean(modelTS$values[0:4],na.rm=TRUE)
      modelTS$values <- modelTS$values / sd(modelTS$values,na.rm=TRUE)
      if (model == 'trace'){
        color = 'fire brick'
      } else if (model == 'hadcm'){
        color = 'goldenrod'
      ...
      compBands$vals <- compBands$vals + 
        geom_line(aes(x=modelTS$time,y=modelTS$values),color=color,size=0.4,alpha=0.5)
    }
  }
  #compBands$vals
  compBandPlt <- ggdraw() 
