...
scatter <- ggplot(data=data,aes(x=averageRange,y=proxyAgreePct,shape=regType,fill=regType,group = regType))+
  ...
  #coord_cartesian(xlim=c(0,20), ylim=c(0,100),expand	=FALSE) +
  labs(y='% of Proxy Records With the Same Sign 6-0 ka Anomaly as the Simulated 6-0 ka Anomalies',x='Absolute Value of Simulated 6-0 ka Anomalies (mm/day)')+
...
z <- cowplot::ggdraw(ggplot() + theme(plot.background= element_rect(colour='White',fill='White'),
