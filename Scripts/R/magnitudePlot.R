...
var <- 'p-e'
proxyDataAgree <- read.csv(file.path(dataDir,'Data','proxyMetaData_HC.csv'))
proxyRegionTS <- read.csv(file.path(dataDir,'Data','RegionComposites','HC','MedianTSbyRegion.csv'))
hadcmRegionTS <- read.csv(file.path(dataDir,'Data','Model','RegionTS',paste('regional_',var,'_ANN_hadcm.csv',sep='')))
traceRegionTS <- read.csv(file.path(dataDir,'Data','Model','RegionTS',paste('regional_',var,'_ANN_trace.csv',sep='')))
...
ggsave(plot=plt, width = 6.5, height = 3.75, dpi = 600,device='png',
       filename = paste(file.path(dataDir,'Figures','magnitudePlot.png',sep='')))
