...
pltRolors <- c()
pltColors <- c('#fcde9c','#faa476','#f0746e','#dc3977','#b9257a','#7c1d6f','black','grey','grey')
...
plt <-ggplot()+
   ...
   theme_bw() + 
   theme(legend.position='none',panel.grid = element_blank(),text = element_text(family=figFont,size=12))
...
```{r}
IPCC <- TRUE
pltColors<-c('#64E5E2','#1985E4','#09E33F','#4BCC99','#4DAA74','#C123A4','#963C8D','#C4600B','#4B3079','black')
pltColors<-c('#4BCC99','#4DAA74','#4B3079','#1985E4','#C123A4','#64E5E2','#963C8D','#09E33F','#C4600B')

refregions <-  IPCC_WGI_reference_regions_v4
countries  <- rworldmap::getMap("less islands")
dataTable <- fortify(subset(refregions, Acronym %in% names(NHmidlat)))
...
map <- ggplot() +
 ...
 geom_map(data=countries,  map=fortify(countries),  
  ...
  coord_fixed(1,ylim=c(20,80)) + 
  theme_void() +
  geom_map(data=fortify(subset(refregions, Acronym %in% names(NHmidlat))), 
           map=dataTable, alpha=0.6, size=0.1, color='black' , 
           aes(x=long, y=lat, group=group, map_id=id, fill=as.factor(id))) +
