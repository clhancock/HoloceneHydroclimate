...
library(plyr )
df <- na.omit(data)
df[nrow(df)+1,] <- data.frame('For Legend',' Proxy Sites',0,0,0,0,0,0,0,0)
...
scatter <- ggplot(data=data,aes(x=averageRange,y=proxyAgreePct,shape=regType,fill=regType,group = regType))+
  ...
  scale_fill_manual(values=c('White',nml,sml,polar,monsoonC,tropicC))+
  scale_shape_manual(values=c(1,24,25,23,21,22))+
  theme_bw()+
  #scale_x_continuous(limits=c(0,1000))+
  coord_cartesian(xlim=c(0,0.28), ylim=c(0,100),expand	=FALSE) +
  labs(y='Proxy Agreement in Direction of Simulated Anomaly \n (% of proxy records within each region) ',
